...
(function($){$.flexslider=function(el,options){var slider=$(el);$.data(el,"flexslider",slider);slider.init=function(){slider.vars=$.extend({},$.flexslider.defaults,options);$.data(el,"flexsliderInit",true);slider.container=$(".slides",slider).first();slider.slides=$(".slides:first > li",slider);slider.count=slider.slides.length;slider.animating=false;slider.currentSlide=slider.vars.slideToStart;slider.animatingTo=slider.currentSlide;slider.atEnd=slider.currentSlide==0?true:false;slider.eventType=
"ontouchstart"in document.documentElement?"touchstart":"click";slider.cloneCount=0;slider.cloneOffset=0;slider.manualPause=false;slider.vertical=slider.vars.slideDirection=="vertical";slider.prop=slider.vertical?"top":"marginLeft";slider.args={};slider.transitions="webkitTransition"in document.body.style;if(slider.transitions)slider.prop="-webkit-transform";if(slider.vars.controlsContainer!=""){slider.controlsContainer=$(slider.vars.controlsContainer).eq($(".slides").index(slider.container));slider.containerExists=
slider.controlsContainer.length>0}if(slider.vars.manualControls!=""){slider.manualControls=$(slider.vars.manualControls,slider.containerExists?slider.controlsContainer:slider);slider.manualExists=slider.manualControls.length>0}if(slider.vars.randomize){slider.slides.sort(function(){return Math.round(Math.random())-0.5});slider.container.empty().append(slider.slides)}if(slider.vars.animation.toLowerCase()=="slide"){if(slider.transitions)slider.setTransition(0);slider.css({"overflow":"hidden"});if(slider.vars.animationLoop){slider.cloneCount=
2;slider.cloneOffset=1;slider.container.append(slider.slides.filter(":first").clone().addClass("clone")).prepend(slider.slides.filter(":last").clone().addClass("clone"))}slider.newSlides=$(".slides:first > li",slider);var sliderOffset=-1*(slider.currentSlide+slider.cloneOffset);if(slider.vertical){slider.newSlides.css({"display":"block","width":"100%","float":"left"});slider.container.height((slider.count+slider.cloneCount)*200+"%").css("position","absolute").width("100%");setTimeout(function(){slider.css({"position":"relative"}).height(slider.slides.filter(":first").height());
...
37)return;else{if(event.keyCode==39)var target=slider.getTarget("next");else if(event.keyCode==37)var target=slider.getTarget("prev");if(slider.canAdvance(target))slider.flexAnimate(target,slider.vars.pauseOnAction)}}$(document).bind("keyup",keyboardMove)}if(slider.vars.mousewheel){slider.mousewheelEvent=/Firefox/i.test(navigator.userAgent)?"DOMMouseScroll":"mousewheel";slider.bind(slider.mousewheelEvent,function(e){e.preventDefault();e=e?e:window.event;var wheelData=e.detail?e.detail*-1:e.originalEvent.wheelDelta/
...
slider.args[slider.prop]=-1*(slider.currentSlide+slider.cloneOffset)*slider.width()+"px";if(slider.transitions){slider.setTransition(0);slider.args[slider.prop]=slider.vertical?"translate3d(0,"+slider.args[slider.prop]+",0)":"translate3d("+slider.args[slider.prop]+",0,0)"}slider.container.css(slider.args)}});slider.vars.start(slider)};slider.flexAnimate=function(target,pause){if(!slider.animating&&slider.is(":visible")){slider.animating=true;slider.animatingTo=target;slider.vars.before(slider);if(pause)slider.pause();
if(slider.vars.controlNav)slider.controlNav.removeClass("active").eq(target).addClass("active");slider.atEnd=target==0||target==slider.count-1?true:false;if(!slider.vars.animationLoop&&slider.vars.directionNav)if(target==0)slider.directionNav.removeClass("disabled").filter(".prev").addClass("disabled");else if(target==slider.count-1)slider.directionNav.removeClass("disabled").filter(".next").addClass("disabled");else slider.directionNav.removeClass("disabled");if(!slider.vars.animationLoop&&target==
slider.count-1){slider.pause();slider.vars.end(slider)}if(slider.vars.animation.toLowerCase()=="slide"){var dimension=slider.vertical?slider.slides.filter(":first").height():slider.slides.filter(":first").width();if(slider.currentSlide==0&&target==slider.count-1&&slider.vars.animationLoop&&slider.direction!="next")slider.slideString="0px";else if(slider.currentSlide==slider.count-1&&target==0&&slider.vars.animationLoop&&slider.direction!="prev")slider.slideString=-1*(slider.count+1)*dimension+"px";
else slider.slideString=-1*(target+slider.cloneOffset)*dimension+"px";slider.args[slider.prop]=slider.slideString;if(slider.transitions){slider.setTransition(slider.vars.animationDuration);slider.args[slider.prop]=slider.vertical?"translate3d(0,"+slider.slideString+",0)":"translate3d("+slider.slideString+",0,0)";slider.container.css(slider.args).one("webkitTransitionEnd transitionend",function(){slider.wrapup(dimension)})}else slider.container.animate(slider.args,slider.vars.animationDuration,function(){slider.wrapup(dimension)})}else{slider.slides.eq(slider.currentSlide).fadeOut(slider.vars.animationDuration);
slider.slides.eq(target).fadeIn(slider.vars.animationDuration,function(){slider.wrapup()})}}};slider.wrapup=function(dimension){if(slider.vars.animation=="slide")if(slider.currentSlide==0&&slider.animatingTo==slider.count-1&&slider.vars.animationLoop){slider.args[slider.prop]=-1*slider.count*dimension+"px";if(slider.transitions){slider.setTransition(0);slider.args[slider.prop]=slider.vertical?"translate3d(0,"+slider.args[slider.prop]+",0)":"translate3d("+slider.args[slider.prop]+",0,0)"}slider.container.css(slider.args)}else if(slider.currentSlide==
slider.count-1&&slider.animatingTo==0&&slider.vars.animationLoop){slider.args[slider.prop]=-1*dimension+"px";if(slider.transitions){slider.setTransition(0);slider.args[slider.prop]=slider.vertical?"translate3d(0,"+slider.args[slider.prop]+",0)":"translate3d("+slider.args[slider.prop]+",0,0)"}slider.container.css(slider.args)}slider.animating=false;slider.currentSlide=slider.animatingTo;slider.vars.after(slider)};slider.animateSlides=function(){if(!slider.animating)slider.flexAnimate(slider.getTarget("next"))};
slider.pause=function(){clearInterval(slider.animatedSlides);if(slider.vars.pausePlay)slider.pausePlay.removeClass("pause").addClass("play").text(slider.vars.playText)};slider.resume=function(){slider.animatedSlides=setInterval(slider.animateSlides,slider.vars.slideshowSpeed);if(slider.vars.pausePlay)slider.pausePlay.removeClass("play").addClass("pause").text(slider.vars.pauseText)};slider.canAdvance=function(target){if(!slider.vars.animationLoop&&slider.atEnd)if(slider.currentSlide==0&&target==slider.count-
1&&slider.direction!="next")return false;else if(slider.currentSlide==slider.count-1&&target==0&&slider.direction=="next")return false;else return true;else return true};slider.getTarget=function(dir){slider.direction=dir;if(dir=="next")return slider.currentSlide==slider.count-1?0:slider.currentSlide+1;else return slider.currentSlide==0?slider.count-1:slider.currentSlide-1};slider.setTransition=function(dur){slider.container.css({"-webkit-transition-duration":dur/1E3+"s"})};slider.init()};$.flexslider.defaults=
{animation:"fade",slideDirection:"horizontal",slideshow:true,slideshowSpeed:7E3,animationDuration:600,directionNav:true,controlNav:true,keyboardNav:true,mousewheel:false,prevText:"Previous",nextText:"Next",pausePlay:false,pauseText:"Pause",playText:"Play",randomize:false,slideToStart:0,animationLoop:true,pauseOnAction:true,pauseOnHover:false,controlsContainer:"",manualControls:"",start:function(){},before:function(){},after:function(){},end:function(){}};$.fn.flexslider=function(options){return this.each(function(){if($(this).find(".slides > li").length==
1)$(this).find(".slides > li").fadeIn(400);else if($(this).data("flexsliderInit")!=true)new $.flexslider(this,options)})}})(jQuery);
